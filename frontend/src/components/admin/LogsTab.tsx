  ...
  return (
    <Card className="p-4 sm:p-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-muted">{filteredLog.length} / {log.length}</span>
      </div>
          ...
          <div className="block sm:hidden space-y-3 p-4">
            {pageItems.map((entry, i) => (
              <MobileCard key={page * ACTIVITY_PAGE_SIZE + i}>
                ...
                <CardField label={t('adminDetail')} value={<span className="text-xs text-muted truncate max-w-[200px] inline-block">{entry.detail}</span>} />
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('syncDate')}</th>
                  <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('adminUser')}</th>
                  <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('adminAction')}</th>
                  <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('adminDetail')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {pageItems.map((entry, i) => (
                  <tr key={page * ACTIVITY_PAGE_SIZE + i} className="hover:bg-surface">
                    <td className="whitespace-nowrap px-4 py-2 text-muted">{formatDateTime(entry.timestamp, locale)}</td>
                    ...
                    <td className="max-w-[400px] truncate px-4 py-2 text-xs text-muted">{entry.detail}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {totalPages > 1 && (
            <div className="mt-4 flex items-center justify-center gap-3">
              <button
                ...
                disabled={page === 0}
                className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-semibold text-muted transition hover:bg-surface disabled:opacity-30"
              >
                <ChevronLeft size={14} />
                {t('pagePrev')}
              </button>
        ...
        <div className="block sm:hidden space-y-3 p-4">
          {history.slice(0, 50).map((entry, i) => (
            <MobileCard key={i}>
              ...
              <CardField label={t('adminBrowser')} value={<span className="text-xs text-muted truncate max-w-[200px] inline-block">{entry.user_agent}</span>} />
              <CardField label={t('syncDate')} value={formatDateTime(entry.timestamp, locale)} />
            </MobileCard>
          ))}
        </div>
          ...
          <table className="w-full min-w-[600px] text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('syncDate')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('adminUser')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('adminRole')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">IP</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted">{t('adminBrowser')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {history.slice(0, 50).map((entry, i) => (
                <tr key={i} className="hover:bg-surface">
                  ...
                  <td className="whitespace-nowrap px-4 py-2 font-mono text-xs text-muted">{entry.ip}</td>
                  <td className="max-w-[300px] truncate px-4 py-2 text-xs text-muted">{entry.user_agent}</td>
                </tr>
              ))}
            </tbody>
          </table>
