...
import { asDriverId } from "../types/common.js";
 ...
 */
export function buildDriverDays(
  activities: readonly Activity[],
  time_zone: string,
): DriverDay[] {
  if (activities.length === 0) return [];
  const driver_id = (activities[0] as Activity).driver_id;
  ...
  for (const [key, list] of [...buckets.entries()].sort()) {
    const sample = list[0] as Activity;
...
function sumByKind(xs: readonly Activity[], kind: Activity["kind"]): number {
  let ms = 0;
  for (const a of xs) {
    if (a.kind === kind) ms += a.end.getTime() - a.start.getTime();
  }
...
export function buildDriverWeeks(
  ...
  const driver_id = (days[0] as DriverDay).driver_id;
  const buckets = new Map<string, DriverDay[]>();
  ...
  const out: DriverWeek[] = [];
  for (const [key, list] of [...buckets.entries()].sort()) {
    const [yearStr, weekStr] = key.split("-W");
    const iso_year = Number(yearStr);
    const iso_week = Number(weekStr);
    const first = list[0] as DriverDay;
    const last = list[list.length - 1] as DriverDay;
    ...
    out.push({
      ...
      total_working_minutes: list.reduce(
        (s, d) => s + d.working_minutes + d.driving_minutes,
        0,
      ),
    });
  }
...
function isoWeekParts(date: Date): { iso_year: number; iso_week: number } {
...
function mondayOfIsoWeek(iso_year: number, iso_week: number, time_zone: string): Date {
 ...
 */
export function rollingTwoWeekDriving(
  weeks: readonly DriverWeek[],
): { iso_year: number; iso_week: number; minutes: number }[] {
  return weeks.map((w, i) => {
    const prev = i > 0 ? (weeks[i - 1] as DriverWeek).total_driving_minutes : 0;
    return {
      iso_year: w.iso_year,
      iso_week: w.iso_week,
