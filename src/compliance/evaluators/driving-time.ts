...
interface DailyLogic {
  normal_limit_minutes: number;
  extended_limit_minutes: number;
  allowed_extensions_per_week: number;
}
interface WeeklyLogic {
  weekly_limit_minutes: number;
}
interface TwoWeekLogic {
  two_week_limit_minutes: number;
}

function readDailyLogic(rule: Rule): DailyLogic | null {
  const l = rule.logic as Partial<DailyLogic> | undefined;
  if (!l || typeof l.normal_limit_minutes !== "number") return null;
  return {
...
function readWeeklyLogic(rule: Rule): WeeklyLogic | null {
...
function readTwoWeekLogic(rule: Rule): TwoWeekLogic | null {
  const l = rule.logic as Partial<TwoWeekLogic> | undefined;
 ...
 */
export const drivingTimeEvaluator: Evaluator = {
  ...
  evaluate(ctx: EvaluatorContext): readonly RuleEvaluation[] {
    ...
    if (ruleDaily) out.push(evalDaily(ctx, ruleDaily, days, weeks));
    if (ruleWeekly) out.push(evalWeekly(ctx, ruleWeekly, weeks));
    if (ruleTwoWeek) out.push(evalTwoWeek(ctx, ruleTwoWeek, weeks));
...
function evalDaily(
  ...
  if (days.length === 0) return { status: "compliant", rule_id: rule.rule_id };

  // Track how many extensions each ISO week has already "used".
  const extensionsByWeek = new Map<string, number>();
  for (const w of weeks) extensionsByWeek.set(weekKey(w.iso_year, w.iso_week), 0);
  ...
  for (const day of sorted) {
    if (day.driving_minutes <= logic.normal_limit_minutes) continue;
  ...
  const violations = weeks
    ...
    .map((w) =>
      buildViolation({
        ...
        unit: "minutes",
        source_activity_ids: w.days.flatMap((d) =>
          d.activities.filter((a) => a.kind === "DRIVING").map((a) => a.id),
        ),
        confidence: 1,
        rule_set_version: ruleSetVersionFor(ctx.rules, rule.rule_id),
      }),
  ...
  const violations = totals
    ...
    .map(({ t, week }) =>
      buildViolation({
        ...
        fines: ctx.fines,
        driver_id: ctx.timeline.driver_id,
        start_time: (week as DriverWeek).window.start,
        end_time: (week as DriverWeek).window.end,
...
function isoWeekFor(day: DriverDay): { key: string; iso_year: number; iso_week: number } {
