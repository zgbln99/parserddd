 ...
 *    slot data.
 */
export const cardEvaluator: Evaluator = {
  rule_ids: [
...
function drivingWithoutCard(ctx: EvaluatorContext): RuleEvaluation | null {
  const rule = ctx.rules.by_id.get(RULE_DRIVING_NO_CARD);
  if (!rule) return null;

  // Card-out windows = gaps in the card-session sequence within the range.
  const cardOut: readonly TimeInterval[] = ctx.timeline.card_sessions.length === 0
    ? [ctx.timeline.range]
    : findGaps(ctx.timeline.range, ctx.timeline.card_sessions);

  const offenders = ctx.timeline.activities.filter(
    (a) =>
      a.kind === "DRIVING" &&
      (!a.card_inserted ||
  ...
  const violations = offenders.map((a) =>
    buildViolation({
      rule,
      fines: ctx.fines,
...
function cardRemovedTooEarly(ctx: EvaluatorContext): RuleEvaluation | null {
  ...
  for (const session of ctx.timeline.card_sessions) {
    const next = ctx.timeline.activities.find(
      (a) => a.start.getTime() >= session.end.getTime(),
    );
    if (!next) continue;
...
function wrongSlot(ctx: EvaluatorContext): RuleEvaluation | null {
  const rule = ctx.rules.by_id.get(RULE_WRONG_SLOT);
  if (!rule) return null;

  const offenders = ctx.timeline.activities.filter(
    (a) => a.kind === "DRIVING" && a.slot === "CO_DRIVER",
  );
  if (offenders.length === 0) {
...
function missingCoDriver(ctx: EvaluatorContext): RuleEvaluation | null {
  const rule = ctx.rules.by_id.get(RULE_MISSING_CO_DRIVER);
  if (!rule) return null;

  const declared = ctx.timeline.activities.filter((a) =>
    a.notes.some((n) => n === "multi_manning" || n === "MEHRFAHRER" || n === "ZAŁOGA"),
  );
  ...
  const first = declared[0] as (typeof declared)[number];
  const last = declared[declared.length - 1] as (typeof declared)[number];
  const violations = [
    buildViolation({
      rule,
      fines: ctx.fines,
