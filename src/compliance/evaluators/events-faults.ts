...
import type { TachographEvent } from "../types/timeline.js";
 ...
 */
export const eventsFaultsEvaluator: Evaluator = {
...
function isEvent(t: string): boolean {
  return t.startsWith("EVENT_") || t.startsWith("E_");
}
function isFault(t: string): boolean {
  return t.startsWith("FAULT_") || t.startsWith("F_");
}
function isManipulation(t: string): boolean {
...
function classify(
  ...
  const violations = offenders.map((e: TachographEvent) =>
    buildViolation({
      rule,
      fines: ctx.fines,
...
function printout(ctx: EvaluatorContext): RuleEvaluation | null {
  const rule = ctx.rules.by_id.get(RULE_PRINTOUT);
  if (!rule) return null;

  const requiringPrintout = ctx.timeline.events.filter(
    (e) => isFault(e.type) || isManipulation(e.type),
  );
