...
interface WeeklyLogic {
  average_limit_hours: number;
  maximum_limit_hours: number;
  averaging_period_weeks: number;
}
interface BreakLogic {
  break_after_6h_minutes: number;
  break_after_9h_minutes: number;
  minimum_split_minutes: number;
}
interface DailyLogic {
  max_daily_minutes: number;
}
interface NightLogic {
  night_window_start_hour: number;
  night_window_end_hour: number;
  max_minutes_on_night_day: number;
}

function readWeekly(rule: Rule): WeeklyLogic | null {
  const l = rule.logic as Partial<WeeklyLogic> | undefined;
  if (
    !l ||
...
function readBreaks(rule: Rule): BreakLogic | null {
  const l = rule.logic as Partial<BreakLogic> | undefined;
  if (
    !l ||
...
function readDaily(rule: Rule): DailyLogic | null {
  const l = rule.logic as Partial<DailyLogic> | undefined;
  return l && typeof l.max_daily_minutes === "number"
    ? { max_daily_minutes: l.max_daily_minutes }
    : null;
}

function readNight(rule: Rule): NightLogic | null {
  const l = rule.logic as Partial<NightLogic> | undefined;
  if (
    !l ||
 ...
 */
export const workingTimeEvaluator: Evaluator = {
  ...
  evaluate(ctx: EvaluatorContext): readonly RuleEvaluation[] {
    ...
    if (rWeekly) out.push(evalWeekly(ctx, rWeekly, weeks));
    if (rBreaks) out.push(evalBreaks(ctx, rBreaks, days));
    if (rDaily) out.push(evalDaily(ctx, rDaily, days));
    if (rNight) out.push(evalNight(ctx, rNight, days));
    return out;
  },
};
...
function evalWeekly(
  ...
  rule: Rule,
  weeks: ReturnType<typeof buildDriverWeeks>,
): RuleEvaluation {
...
function evalBreaks(
  ctx: EvaluatorContext,
  rule: Rule,
  days: ReturnType<typeof buildDriverDays>,
): RuleEvaluation {
  ...
  for (const day of days) {
    ...
    let runMinutes = 0;
    const flush = () => {
      if (runMinutes >= logic.minimum_split_minutes) qualifying += runMinutes;
      runMinutes = 0;
    };
    ...
    if (qualifying >= required) continue;
...
function evalDaily(
  ...
  for (const day of days) {
    const work = day.driving_minutes + day.working_minutes;
    if (work <= logic.max_daily_minutes) continue;
    violations.push(
      buildViolation({
        rule,
        fines: ctx.fines,
...
function evalNight(
  ...
  const violations = [] as ReturnType<typeof buildViolation>[];
  for (const day of days) {
    const isNight = day.activities.some((a) =>
      activityIntersectsNightWindow(a, ctx.time_zone, logic),
    );
    if (!isNight) continue;
    const work = day.driving_minutes + day.working_minutes;
    if (work <= logic.max_minutes_on_night_day) continue;
    violations.push(
      buildViolation({
        rule,
        fines: ctx.fines,
...
function activityIntersectsNightWindow(
  a: Activity,
  time_zone: string,
  logic: NightLogic,
): boolean {
  if (a.kind !== "WORK" && a.kind !== "DRIVING") return false;
  ...
  for (let t = localStartMs; t < localEndMs; t += 60 * 60 * 1000) {
    ...
    if (
      hour >= logic.night_window_start_hour &&
