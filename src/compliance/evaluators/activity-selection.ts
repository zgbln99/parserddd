 ...
 */
export const activitySelectionEvaluator: Evaluator = {
...
function restDuringWork(ctx: EvaluatorContext): RuleEvaluation | null {
  const rule = ctx.rules.by_id.get(RULE_REST_DURING_WORK);
  if (!rule) return null;

  const movingSessions = ctx.timeline.card_sessions.filter(
    (s) =>
      s.start_odometer_km != null &&
...
function outMisuse(ctx: EvaluatorContext): RuleEvaluation | null {
  const rule = ctx.rules.by_id.get(RULE_OUT_MISUSE);
  if (!rule) return null;
  const max = (rule.logic as { max_consecutive_minutes?: number } | undefined)
    ?.max_consecutive_minutes ?? 720;

  // Coalesce consecutive OUT activities and check total minutes per run.
  type Run = { start: Date; end: Date; ids: import("../types/common.js").ActivityId[] };
  const runs: Run[] = [];
  ...
  const offenders = runs.filter(
    (r) => Math.round((r.end.getTime() - r.start.getTime()) / 60_000) > max,
  );
  ...
  const violations = offenders.map((r) =>
    buildViolation({
      ...
      allowed_value: max,
      excess_value:
        Math.round((r.end.getTime() - r.start.getTime()) / 60_000) - max,
      unit: "minutes",
...
function availabilityInsteadOfWork(ctx: EvaluatorContext): RuleEvaluation | null {
