import type { ManualEntry } from "../types/timeline.js";
...
export const manualEntryEvaluator: Evaluator = {
...
function evaluateMissing(ctx: EvaluatorContext): RuleEvaluation {
  ...
  const meaningful = uncovered.filter(
    (g) => g.end.getTime() - g.start.getTime() >= 60_000,
  );
  ...
  const violations = meaningful.map((gap) =>
    buildViolation({
      rule,
      fines: ctx.fines,
...
function evaluateIncomplete(ctx: EvaluatorContext): RuleEvaluation {
  ...
  const violations = incomplete.map((m: ManualEntry) =>
    buildViolation({
      rule,
      fines: ctx.fines,
      driver_id: ctx.timeline.driver_id,
...
function subtractIntervals(
  ...
  for (const interval of sortIntervals(a)) {
    ...
    for (const sub of subtractors) {
      ...
      if (sub.start >= interval.end) break;
      ...
      if (sub.end > cursor) cursor = sub.end > interval.end ? interval.end : sub.end;
