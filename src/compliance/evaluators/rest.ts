...
interface DailyRestLogic {
  ...
  /** EU 561 Art. 4(g) split: first block (default 3h). */
  split_first_min_minutes: number;
  /** EU 561 Art. 4(g) split: second block (default 9h). */
  split_second_min_minutes: number;
}
...
function readDailyRestLogic(rule: Rule): DailyRestLogic | null {
  ...
  const ll = l as Partial<DailyRestLogic>;
  return {
...
/**
 * A "daily rest event" is a moment in time at which a driver is deemed to
 * have completed a daily rest period. It can be:
 *  - the END of a single rest run >= reduced_daily_rest_minutes, or
 *  - the END of the second block of a valid 3+9 split (Art. 4(g)).
 *
 * Each event records the equivalent rest minutes so the evaluator can
 * decide whether it counts as regular (11h) or reduced (consumes a budget).
 */
interface DailyRestEvent {
  readonly start: Date;
  readonly end: Date;
...
function findDailyRestEvents(
  rests: readonly { start: Date; end: Date; minutes: number }[],
  logic: DailyRestLogic,
): DailyRestEvent[] {
  const events: DailyRestEvent[] = [];
  const consumed = new Set<number>();

  for (let i = 0; i < rests.length; i++) {
    if (consumed.has(i)) continue;
    const r = rests[i] as (typeof rests)[number];
    ...
    if (r.minutes >= logic.split_first_min_minutes) {
      ...
      for (let j = i + 1; j < rests.length; j++) {
        const s = rests[j] as (typeof rests)[number];
        if (s.end.getTime() > windowEnd) break;
...
function evalDaily(ctx: EvaluatorContext, rule: Rule): RuleEvaluation {
  ...
  const firstWork = ctx.timeline.activities.find(
    (a) => a.kind === "WORK" || a.kind === "DRIVING" || a.kind === "AVAILABILITY",
  );
  if (!firstWork) {
  ...
  const failedAttempts = rests.filter(
    (r) =>
      r.minutes >= ATTEMPT_THRESHOLD &&
  ...
  for (const e of events) {
    if (e.minutes >= logic.regular_daily_rest_minutes) continue;
...
function evalWeekly(ctx: EvaluatorContext, rule: Rule): RuleEvaluation {
  ...
  const first = rests[0] as (typeof rests)[number];
  ...
  for (let i = 0; i < rests.length - 1; i++) {
    const prev = rests[i] as (typeof rests)[number];
    const next = rests[i + 1] as (typeof rests)[number];
  ...
  const last = rests[rests.length - 1] as (typeof rests)[number];
