...
const day = (offset: number, drivingHours: number) =>
  activity(
    `2026-03-${String(2 + offset).padStart(2, "0")}T06:00:00Z`,
    `2026-03-${String(2 + offset).padStart(2, "0")}T${String(6 + drivingHours).padStart(2, "0")}:00:00Z`,
    { kind: "DRIVING" },
  );
...
describe("driving-time evaluator", () => {
  ...
  it("flags weekly driving > 56h", async () => {
    ...
    const r = evaluateAll(ctx, [drivingTimeEvaluator]);
    expect(
      r.violations.filter((v) => v.rule_id === "EU_561_ART6_WEEKLY_DRIVING"),
    ).toHaveLength(1);
  });
  ...
  it("flags two-week driving > 90h", async () => {
    ...
    for (let week = 0; week < 2; week++) {
      ...
      for (let d = 0; d < 6; d++) {
        ...
        acts.push(
          activity(
            `2026-03-${String(dayDate).padStart(2, "0")}T06:00:00Z`,
            `2026-03-${String(dayDate).padStart(2, "0")}T14:30:00Z`,
            { kind: "DRIVING" },
          ),
        );
      }
    ...
    const r = evaluateAll(ctx, [drivingTimeEvaluator]);
    expect(
      r.violations.filter((v) => v.rule_id === "EU_561_ART6_TWO_WEEK_DRIVING").length,
    ).toBeGreaterThan(0);
  });
});
