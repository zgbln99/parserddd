...
describe("signatures + dispute + correction workflow", () => {
  ...
  it("rejects illegal transitions", () => {
    const v: Violation = { ...sampleViolation(), status: "signed" };
    expect(() =>
      signViolation({
        violation: v,
        driver_id: v.driver_id,
  ...
  it("corrections produce a delta with from/to fields", () => {
    const v: Violation = { ...sampleViolation(), status: "disputed" };
    ...
    const delta = out.delta as Record<string, { from: number; to: number }>;
