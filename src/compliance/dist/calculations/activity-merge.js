 ...
 */
export function mergeActivities(candidates) {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
        ...
        const overlapping = candidates.filter((c) => c.start.getTime() <= start.getTime() && c.end.getTime() >= end.getTime());
...
function materialize(c, start, end, ordinal) {
    return {
 ...
 */
export function fillGapsAsUnknown(activities, range_start, range_end, template) {
...
function makeUnknown(start, end, t, ordinal) {
    return {
...
//# sourceMappingURL=activity-merge.js.map