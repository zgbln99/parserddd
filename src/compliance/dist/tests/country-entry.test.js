...
async function makeCtx(opts) {
    ...
    const timeline = normalizeTimeline({
        ...
        card_sessions: opts.card_sessions.map((o, i) => cardSession(`2026-01-10T0${5 + i}:00:00Z`, `2026-01-10T0${9 + i}:00:00Z`, o ?? {})),
        manual_entries: [],
        events: [],
        printouts: [],
    });
...
describe("country-entry evaluator", () => {
    it("flags a card session missing start_country", async () => {
        ...
        const startViolations = result.violations.filter((v) => v.rule_id === "EU_165_MISSING_START_COUNTRY");
    ...
    it("flags a card session missing end_country", async () => {
        ...
        const result = evaluateAll(ctx);
        const endViolations = result.violations.filter((v) => v.rule_id === "EU_165_MISSING_END_COUNTRY");
    ...
    it("emits no violation when both countries are present", async () => {
        ...
        const result = evaluateAll(ctx);
        expect(result.violations.filter((v) => v.rule_id === "EU_165_MISSING_START_COUNTRY" ||
            v.rule_id === "EU_165_MISSING_END_COUNTRY")).toEqual([]);
    });
...
//# sourceMappingURL=country-entry.test.js.map