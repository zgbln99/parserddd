...
async function makeCtx(opts) {
    ...
    const timeline = normalizeTimeline({
        ...
        range_end: RANGE_END,
        activity_candidates: opts.activities ??
            [
...
describe("manual-entry evaluator", () => {
    it("flags a card-out window with no manual entry", async () => {
        ...
        const v = result.violations.filter((x) => x.rule_id === "EU_165_MISSING_MANUAL_ENTRY");
    ...
    it("counts as compliant when manual entry covers the whole card-out window", async () => {
        ...
        const result = evaluateAll(ctx);
        expect(result.violations.filter((x) => x.rule_id === "EU_165_MISSING_MANUAL_ENTRY")).toEqual([]);
    });
    it("flags an incomplete manual entry", async () => {
        ...
        const result = evaluateAll(ctx);
        const incomplete = result.violations.filter((x) => x.rule_id === "EU_165_INCOMPLETE_MANUAL_ENTRY");
...
//# sourceMappingURL=manual-entry.test.js.map