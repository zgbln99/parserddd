...
describe("card evaluator", () => {
    it("flags driving without card_inserted=true", async () => {
        ...
        expect(r.violations.filter((v) => v.rule_id === "EU_165_DRIVING_WITHOUT_CARD")).toHaveLength(1);
    });
    it("flags wrong slot (driving on co-driver slot)", async () => {
        ...
        const r = evaluateAll(ctx, [cardEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "EU_165_WRONG_SLOT")).toHaveLength(1);
    });
    it("flags card removed too early when shift continues", async () => {
        ...
        const r = evaluateAll(ctx, [cardEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "EU_165_CARD_REMOVED_TOO_EARLY")
    ...
    it("flags missing co-driver when multi_manning declared", async () => {
        ...
        const r = evaluateAll(ctx, [cardEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "EU_165_MISSING_CO_DRIVER")).toHaveLength(1);
    });
});
