...
describe("activity-selection evaluator", () => {
    it("flags rest selected during a card session that recorded movement", async () => {
        ...
        expect(r.violations.filter((v) => v.rule_id === "EU_165_REST_DURING_WORK")).toHaveLength(1);
    });
    it("flags OUT misuse on long OUT runs", async () => {
        ...
        const r = evaluateAll(ctx, [activitySelectionEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "EU_165_OUT_MISUSE").length).toBeGreaterThan(0);
    });
    it("flags availability sandwich pattern", async () => {
        ...
        const r = evaluateAll(ctx, [activitySelectionEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "EU_165_AVAILABILITY_INSTEAD_OF_WORK")
            .length).toBeGreaterThan(0);
    });
});
//# sourceMappingURL=activity-selection.test.js.map