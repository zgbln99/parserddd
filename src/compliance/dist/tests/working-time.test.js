...
describe("working-time evaluator (KrFArbZG)", () => {
    it("flags daily work above 10h", async () => {
        ...
        expect(r.violations.filter((v) => v.rule_id === "DE_KRFARBZG_DAILY_WORKING_TIME")).toHaveLength(1);
    });
    it("flags missing 30min break after 6h work", async () => {
        ...
        const r = evaluateAll(ctx, [workingTimeEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "DE_KRFARBZG_BREAKS")).toHaveLength(1);
    });
    it("flags night work day above 10h", async () => {
        ...
        const r = evaluateAll(ctx, [workingTimeEvaluator]);
        expect(r.violations.filter((v) => v.rule_id === "DE_KRFARBZG_NIGHT_WORK").length).toBeGreaterThan(0);
    });
});
//# sourceMappingURL=working-time.test.js.map