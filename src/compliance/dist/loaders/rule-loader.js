...
import { formatIssues, validateFineSet, validateRuleSet, } from "../validators/schema-validator.js";
export class RuleLoadError extends Error {
    source_path;
    issues;
    constructor(source_path, message, issues = []) {
 ...
 */
export function loadRuleSetFile(file_path) {
    const abs = resolve(file_path);
    const raw = readFileSync(abs, "utf8");
    const parsed = safeParse(raw, abs);
    const result = validateRuleSet(parsed);
    if (!result.ok) {
        throw new RuleLoadError(abs, "rule set failed schema validation:\n" + formatIssues(result.issues), result.issues);
    }
 ...
 */
export function loadFineSetFile(file_path) {
    const abs = resolve(file_path);
    const raw = readFileSync(abs, "utf8");
    const parsed = safeParse(raw, abs);
    const result = validateFineSet(parsed);
    if (!result.ok) {
        throw new RuleLoadError(abs, "fine set failed schema validation:\n" + formatIssues(result.issues), result.issues);
    }
    const data = result.value;
 ...
 */
export async function loadComplianceBundle(...roots) {
    const ruleSets = [];
    const fineSets = [];
    for (const root of roots) {
        for (const file of await walkYaml(resolve(root))) {
            ...
            if (isFineSetShape(parsed)) {
                ...
                throw new RuleLoadError(file, "YAML does not look like a rule set (missing `rules:`) or fine set (missing `fines:`)");
            }
        }
    }
    return {
...
function isFineSetShape(x) {
    return (typeof x === "object" &&
        x !== null &&
        Array.isArray(x.fines));
}
function isRuleSetShape(x) {
    return (typeof x === "object" &&
        x !== null &&
        Array.isArray(x.rules));
}
export function buildRuleIndex(rule_sets) {
    const by_id = new Map();
    const by_category = new Map();
    for (const set of rule_sets) {
        for (const rule of set.rules) {
            if (by_id.has(rule.rule_id)) {
                throw new RuleLoadError(set.source_path, `duplicate rule_id ${rule.rule_id} (also defined in another rule set)`);
            }
...
export function buildFineIndex(fine_sets) {
    const by_rule_id = new Map();
    for (const set of fine_sets) {
        for (const fine of set.fines) {
            if (by_rule_id.has(fine.violation_reference)) {
                ...
                throw new RuleLoadError(set.source_path, `duplicate fine for ${fine.violation_reference}; multi-jurisdiction fines must live in separate indices`);
            }
