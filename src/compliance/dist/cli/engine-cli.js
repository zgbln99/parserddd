...
import { asDriverId, asVehicleId, asActivityId, } from "../types/common.js";
const D = (x) => {
...
function asActivityCandidate(raw, idx) {
...
function asCardSession(raw, idx) {
    if (typeof raw !== "object" || raw === null) {
    ...
    return {
        ...
        start_country: typeof r.start_country === "string" ? r.start_country : null,
        end_country: typeof r.end_country === "string" ? r.end_country : null,
        start_odometer_km: typeof r.start_odometer_km === "number" ? r.start_odometer_km : null,
        end_odometer_km: typeof r.end_odometer_km === "number" ? r.end_odometer_km : null,
        start: D(r.start),
        end: D(r.end),
    };
}
function asManualEntry(raw, idx) {
    const r = raw;
    if (!r)
        throw new Error(`manual_entries[${idx}] is not an object`);
    return {
        driver_id: asDriverId(String(r.driver_id ?? "")),
...
function asEvent(raw, idx) {
    const r = raw;
    if (!r)
        throw new Error(`events[${idx}] is not an object`);
    return {
...
function asPrintout(raw, idx) {
    const r = raw;
    if (!r)
        throw new Error(`printouts[${idx}] is not an object`);
    return {
        driver_id: r.driver_id == null ? null : asDriverId(String(r.driver_id)),
...
function asAdministrative(raw) {
    const r = raw ?? {};
    const downloads = (r.downloads ?? []).map((d, i) => {
        const dr = d;
        if (!dr)
            throw new Error(`administrative.downloads[${i}] is not an object`);
        return {
    ...
    const inspections = (r.inspections ?? []).map((d, i) => {
        const dr = d;
        if (!dr)
            throw new Error(`administrative.inspections[${i}] is not an object`);
        return {
            vehicle_id: asVehicleId(String(dr.vehicle_id ?? "")),
            performed_at: D(dr.performed_at),
            workshop_card_id: typeof dr.workshop_card_id === "string" ? dr.workshop_card_id : null,
            notes: Array.isArray(dr.notes) ? dr.notes : [],
        };
    });
    const retention = (r.retention ?? []).map((d, i) => {
        const dr = d;
        if (!dr)
            throw new Error(`administrative.retention[${i}] is not an object`);
        return {
            source: dr.source === "VEHICLE_UNIT" ? "VEHICLE_UNIT" : "CARD",
            driver_id: dr.driver_id == null ? null : asDriverId(String(dr.driver_id)),
            vehicle_id: dr.vehicle_id == null ? null : asVehicleId(String(dr.vehicle_id)),
...
async function cmdEvaluate() {
    ...
    if (!input.rules_root)
        fail(2, "missing rules_root");
    if (!input.driver_id)
        fail(2, "missing driver_id");
    if (!input.range?.start || !input.range?.end)
        fail(2, "missing range");
    let bundle;
    try {
    ...
    const driver_id = asDriverId(input.driver_id);
    let timeline;
    try {
...
async function cmdReport(locale) {
    let result;
    try {
...
async function main() {
    ...
    if (cmd === "report") {
        ...
        const locale = (localeArg?.split("=")[1] ?? "de");
