...
import { maxContinuousDrivingMinutes, splitDrivingSegments, } from "../calculations/continuous-driving.js";
const RULE_ID = "EU_561_ART7_BREAK_AFTER_4H30";
function readLogic(rule) {
    const l = rule.logic;
    if (!l ||
 ...
 */
export const breakAfter4h30Evaluator = {
    rule_ids: [RULE_ID],
    evaluate(ctx) {
        ...
        const threshold = rule.logic
            .max_continuous_driving_minutes;
        const segments = splitDrivingSegments(ctx.timeline.activities, logic);
        const offenders = segments.filter((s) => maxContinuousDrivingMinutes(s) > threshold);
...
//# sourceMappingURL=break-4h30.js.map